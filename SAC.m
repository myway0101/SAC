...
%% DATA LOAD 
orig_trial = 100;
trial_samples = 4097; % change this to 2048 for 1/2 samples, 1024 for 1/4 samples
...
%% data setup 4096 * 100
for (k = 1:orig_trial)
    data_A2 = [data_A2; data_A1(:, (k-1)*trial_sample+1:(trial_sample*k))];
    data_B2 = [data_B2; data_B1(:, (k-1)*trial_sample+1:(trial_sample*k))];
    data_C2 = [data_C2; data_C1(:, (k-1)*trial_sample+1:(trial_sample*k))];
    data_D2 = [data_D2; data_D1(:, (k-1)*trial_sample+1:(trial_sample*k))];
    data_E2 = [data_E2; data_E1(:, (k-1)*trial_sample+1:(trial_sample*k))];
end
data_A3 = data_A2';
data_B3 = data_B2';
data_C3 = data_C2';
...
data_A = data_A3(1:trial_samples, :);  % time samples per epoch
data_B = data_B3(1:trial_samples, :);
data_C = data_C3(1:trial_samples, :);
data_D = data_D3(1:trial_samples, :);
data_E = data_E3(1:trial_samples, :);
...
% choose two dataset for classification
...
for L = [3] %choose L1 minimization 2:FISTA, 3:homotopy, 5:Pseudo_inv
        for n = 1:crossval
